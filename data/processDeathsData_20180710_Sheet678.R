...
# Date: 10 July 2018
# Purpose: Import and transform Excel data for sheets 6,7,8, append esimated resident populations and calculate rates and confidence intervals
...
sheet6 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=6, skip=1) %>% 
  clean_names() %>% 
...
sheet7 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=7, skip=1) %>% 
  clean_names() %>% 
...
sheet8 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=8, skip=1) %>% 
  clean_names() %>% 
  filter_all(any_vars(!is.na(.))) %>% # Drop rows with all missing columns
  fill(drug, intent) %>% 
  gather(key=sex_age, value=n, male_age15_64, female_age15_64, all_age15_64, male_age_all, female_age_all, all_age_all) %>% 
...
sheet678 <- bind_rows(sheet6, sheet7, sheet8) %>% 
  mutate(sex = case_when( sex %in% c("all", "All", "Persons") ~ "All",
...
# Note: n's may differ across tables for small numbers of deaths due to confidentialisation
# We will use the first identified count of deaths
d <- distinct(sheet678, year, drug, intent, nature, sex, jurisdiction, age_group, .keep_all=TRUE)
...
dfallpop <- left_join(x = d, y = popmerged) %>% 
  mutate(
...
dfSummary(dfallpop)
View(dfallpop)
# write_csv(dfallpop, "Data/Transformed/2018-07-11_Deaths_Pop_CI_Sheets678.csv")