...
sheet1 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=1, skip=1) %>% 
  clean_names() %>% 
...
sheet2 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=2, skip=1) %>% 
  clean_names() %>% 
  filter_all(any_vars(!is.na(.))) %>% 
  fill(drug, intent, nature, cause_of_death, sex, jurisdiction) %>% 
...
sheet3 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=3, skip=1) %>% 
  clean_names() %>% 
...
sheet4 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=4, skip=1) %>% 
  clean_names() %>% 
  filter_all(any_vars(!is.na(.))) %>% # Drop rows with all missing columns
...
sheet5 <- read_excel("Data/Transformed/ABS Feed transformed.xlsx", sheet=5, skip=1) %>% 
  clean_names() %>% 
  filter_all(any_vars(!is.na(.))) %>% # Drop rows with all missing columns
